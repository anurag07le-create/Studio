/**
 * locationRefStore.js — CRUD for location_refs table
 */
...
const create = (data) => {
  ...
  db.prepare(`
    INSERT INTO location_refs (id, projectId, name, description, referenceImageUrl, traits, createdAt, updatedAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
const getById = (id) => {
  const row = db.prepare('SELECT * FROM location_refs WHERE id = ?').get(id);
  return row ? _parse(row) : null;
};

const listByProject = (projectId) => {
  const rows = db.prepare('SELECT * FROM location_refs WHERE projectId = ? ORDER BY createdAt ASC').all(projectId);
...
const update = (id, data) => {
  ...
  values.push(now);
  values.push(id);
  db.prepare(`UPDATE location_refs SET ${updates.join(', ')} WHERE id = ?`).run(...values);
  return getById(id);
};

const remove = (id) => {
  return db.prepare('DELETE FROM location_refs WHERE id = ?').run(id);
};

const removeByProject = (projectId) => {
  return db.prepare('DELETE FROM location_refs WHERE projectId = ?').run(projectId);
};
